...
FIRST_COUNTING_KICK_COLUMN = 34 #player l1 counting kick log dataframe column position
COLUMNS_UNTIL_NEXT_COUNTING_KICK = 31 # number of columns between counting kicks plus one 
COLUMNS_FROM_X_POS_TO_COUNTING_KICKS = 16 #number of columns between player x position and player counting kicks plus one
COLUMNS_FROM_Y_POS_TO_COUNTING_KICKS = 15 #number of columns between player y position and player counting kicks plus one
...
class DataCollector():
    ...
    # instaciates the teams
		...
		columnCursor = FIRST_COUNTING_KICK_COLLUMN
		...
		while(True):
			...
			for i in range(0, TOTAL_NUMBER_OF_PLAYERS - 1):		
				...
				if(statChanged(logDataFrame, rowNumber,columnCursor)):
					...
					#recent_tackler_tackle_xPos = logDataFrame.iloc[rowCursor, columnCursor - COLUMNS_FROM_X_POS_TO_COUNTING_KICKS] # x position  
					#recent_tackler_tackle_yPos = logDataFrame.iloc[rowCursor, columnCursor - COLUMNS_FROM_Y_POS_TO_COUNTING_KICKS] # y position
