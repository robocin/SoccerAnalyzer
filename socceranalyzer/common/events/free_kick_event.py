...
class FreeKickEvent(AbstractEvent): 
    def __init__(self, 
                match: Match,
                ...
                team_actor: Team):
        """
            A class to represent a Free kick Event in the match

            free_kick_event(match: Match,  cicles: tuple[int, int], players_left: list[Agent2D], players_right: list[Agent2D], team_actor: Team)
    ...
    def position(self) -> Point:
        """
        Returns the position of the ball at the time that the free kick occurred. 
        """
